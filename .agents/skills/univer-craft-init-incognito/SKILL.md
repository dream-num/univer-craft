...
use its `dependencies.env` and `templates/AGENTS.md`. The combined installer and
both Univer Craft source skill directories belong to `$bundle_root`. Never use
either source repository as the target.

Read and follow [Tina Init Incognito](../../../vendor/tina/.agents/skills/tina-init-incognito/SKILL.md)
with the source paths and installer substitution described here.
Retain its explicit target requirement, Git worktree checks, baseline capture,
conflict protection, instruction preservation, rollback, and verification.
