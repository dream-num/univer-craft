...
if [ ! -f "$UNIVER_CRAFT_ROOT/vendor/tina/install.sh" ]; then
  echo "Initialize Tina first: git -C \"$UNIVER_CRAFT_ROOT\" submodule update --init vendor/tina" >&2
  exit 1
fi
...
"$UNIVER_CRAFT_ROOT/vendor/tina/install.sh" "$TARGET"
